...
std::pair<
    std::pair<
        ...
        parlay::sequence<std::pair<indexType, typename Point::distanceType>>>,
    size_t>
filtered_beam_search(const GT &G, const Point p, const PointRange &Points,
                     const QPoint qp, const QPointRange &Q_Points,
                     const parlay::sequence<indexType> starting_points,
                     const QueryParams &QP, bool use_filtering = false) {
    ...
    if (starting_points.size() == 0) {
        std::cout << "beam search expects at least one start point"
                  << std::endl;
        abort();
    }
    ...
    std::vector<id_dist> new_frontier(
        2 * std::max<size_t>(beamSize, starting_points.size()) +
        G.max_degree());
    ...
    // offset into the unvisited_frontier vector (unvisited_frontier[offset] is
    // the next to visit)
    int offset = 0;
    ...
    while (remain > offset && num_visited < QP.limit) {
        ...
        // if using filtering based on lower quality distances measure, then
        // maintain the average of low quality distance to the last point in the
        // frontier (if frontier is full)
        if (use_filtering && frontier_full) {
            ...
            if (filter_threshold_count == 0 || filter_id != id) {
                ...
                for (int i = frontier.size() - width; i < frontier.size(); i++)
                    filter_tail_mean +=
                        Q_Points[frontier[i].first].distance(qp);
        ...
        filtered.clear();
        long num_elts =
            std::min<long>(G[current.first].size(), QP.degree_limit);
        for (indexType i = 0; i < num_elts; i++) {
            auto a = G[current.first][i];
            if (has_been_seen(a) || Points[a].same_as(p))
        ...
        if (use_filtering && frontier_full) {
            for (auto a : pruned) {
                if (frontier_full &&
                    Q_Points[a].distance(qp) >= filter_threshold)
                    continue;
        ...
        // If candidates insufficently full then skip rest of step until
        // sufficiently full. This iproves performance for higher accuracies
        // (e.g. beam sizes of 100+)
        ...
        // and remove any duplicates (to be robust for neighbor lists with
        // duplicates)
        ...
        remain = (std::set_difference(
                      frontier.begin(),
                      frontier.begin() +
                          std::min<long>(frontier.size(), QP.beamSize),
...
beam_search(const Point p, const Graph<indexType> &G, const PointRange &Points,
    ...
    return filtered_beam_search(G, p, Points, p, Points, starting_points, QP,
                                false);
}
...
template <typename indexType, typename Point, typename PointRange, class GT>
std::pair<std::vector<indexType>, typename Point::distanceType> range_search(
    Point p, GT &G, PointRange &Points,
    parlay::sequence<indexType> starting_points,
    typename Point::distanceType radius, typename Point::distanceType radius_2,
    ...
    while (position < result.size()) {
        ...
        for (auto v : unseen) {
            distance_comparisons++;
            if (Points[v].distance(p) <= radius_2) result.push_back(v);
        }
    }
...
parlay::sequence<parlay::sequence<indexType>> beamSearchRandom(
    const PointRange &Query_Points, const Graph<indexType> &G,
    const PointRange &Base_Points, stats<indexType> &QueryStats,
    const QueryParams &QP) {
    ...
    parlay::sequence<parlay::sequence<indexType>> all_neighbors(
        Query_Points.size());
    ...
    parlay::parallel_for(0, Query_Points.size(), [&](size_t i) {
        parlay::sequence<indexType> neighbors =
            parlay::sequence<indexType>(QP.k);
...
parlay::sequence<parlay::sequence<indexType>> searchAll(
    ...
    stats<indexType> &QueryStats, indexType starting_point, QueryParams &QP) {
    parlay::sequence<indexType> start_points = {starting_point};
    return searchAll<PointRange, indexType>(Query_Points, G, Base_Points,
...
parlay::sequence<parlay::sequence<indexType>> searchAll(
    PointRange &Query_Points, Graph<indexType> &G, PointRange &Base_Points,
    stats<indexType> &QueryStats, parlay::sequence<indexType> starting_points,
    QueryParams &QP) {
    if (QP.k > QP.beamSize) {
        std::cout << "Error: beam search parameter Q = " << QP.beamSize
                  << " same size or smaller than k = " << QP.k << std::endl;
    ...
    parlay::parallel_for(0, Query_Points.size(), [&](size_t i) {
        parlay::sequence<indexType> neighbors =
            parlay::sequence<indexType>(QP.k);
        auto [pairElts, dist_cmps] =
            beam_search(Query_Points[i], G, Base_Points, starting_points, QP);
...
template <typename Point, typename QPoint, typename QQPoint,
          ...
          typename indexType>
parlay::sequence<std::pair<indexType, typename Point::distanceType>>
beam_search_rerank(const Point &p, const QPoint &qp, const QQPoint &qqp,
                   const Graph<indexType> &G, const PointRange &Base_Points,
                   const QPointRange &Q_Base_Points,
                   const QQPointRange &QQ_Base_Points,
                   stats<indexType> &QueryStats,
                   const parlay::sequence<indexType> starting_points,
                   const QueryParams &QP, bool stats = true) {
    using dtype = typename Point::distanceType;
    using id_dist = std::pair<indexType, dtype>;
    auto QPP = QP;

    bool use_rerank =
        (Base_Points.params.num_bytes() != Q_Base_Points.params.num_bytes());
    bool use_filtering =
        (Q_Base_Points.params.num_bytes() != QQ_Base_Points.params.num_bytes());
    auto [pairElts, dist_cmps] =
        filtered_beam_search(G, qp, Q_Base_Points, qqp, QQ_Base_Points,
    ...
    if (use_rerank) {
        ...
        for (int i = 0; i < QP.k; i++) results.push_back(pts[i]);
...
// Returns a sequence of nearest neighbors each with their distance
template <typename Point, typename QPoint, typename PointRange,
          typename QPointRange, typename indexType>
std::pair<parlay::sequence<std::pair<indexType, typename Point::distanceType>>,
          indexType>
beam_search_rerank_(const Point &p, const QPoint &qp, const Graph<indexType> &G,
                    const PointRange &Base_Points,
                    const QPointRange &Q_Base_Points, indexType starting_point,
                    const QueryParams &QP) {
    using dtype = typename Point::distanceType;
    using id_dist = std::pair<indexType, dtype>;
    ...
    if (use_rerank) {
        auto [pairElts, dist_cmps] =
            filtered_beam_search(G, qp, Q_Base_Points, qp, Q_Base_Points,
        ...
        auto [pairElts, dist_cmps] =
            beam_search(p, G, Base_Points, starting_point, QP);
...
std::pair<parlay::sequence<std::pair<indexType, typename Point::distanceType>>,
          indexType>
beam_search_rerank__(const Point &p, const QPoint &qp,
                     const Graph<indexType> &G, const PointRange &Base_Points,
                     const QPointRange &Q_Base_Points, indexType starting_point,
                     const QueryParams &QP) {
    using dtype = typename Point::distanceType;
    ...
    bool use_rerank =
        (Base_Points.params.num_bytes() != Q_Base_Points.params.num_bytes());
    auto [pairElts, dist_cmps] = filtered_beam_search(
        G, p, Base_Points, qp, Q_Base_Points, starting_points, QP, use_rerank);
    return std::pair(pairElts.second, dist_cmps);
}
...
//            const QueryParams &QP) {
//   parlay::sequence<indexType> start_points = {starting_point};
//   return qsearchAll<PointRange, QPointRange, indexType>(Query_Points,
//   Q_Query_Points, G, Base_Points, Q_Base_Points, QueryStats, start_points,
//   QP);
// }

template <typename PointRange, typename QPointRange, typename QQPointRange,
          typename indexType>
parlay::sequence<parlay::sequence<indexType>> qsearchAll(
    const PointRange &Query_Points, const QPointRange &Q_Query_Points,
    const QQPointRange &QQ_Query_Points, const Graph<indexType> &G,
    const PointRange &Base_Points, const QPointRange &Q_Base_Points,
    const QQPointRange &QQ_Base_Points, stats<indexType> &QueryStats,
    const indexType starting_point, const QueryParams &QP,
    bool random = false) {
    if (QP.k > QP.beamSize) {
        std::cout << "Error: beam search parameter Q = " << QP.beamSize
    ...
    if (random) {
        ...
        auto indices =
            parlay::tabulate(Query_Points.size(), [&](size_t i) -> indexType {
                auto r = gen[i];
                return dis(r);
            });
        ...
        parlay::parallel_for(0, Query_Points.size(), [&](size_t i) {
            ...
            auto ngh_dist = beam_search_rerank(
                Query_Points[i], Q_Query_Points[i], QQ_Query_Points[i], G,
                Base_Points, Q_Base_Points, QQ_Base_Points, QueryStats,
                starting_points, QP);
            all_neighbors[i] =
                parlay::map(ngh_dist, [](auto &p) { return p.first; });
        });
    } else {
        parlay::sequence<indexType> starting_points = {starting_point};
        parlay::parallel_for(0, Query_Points.size(), [&](size_t i) {
...
//             Graph<indexType> &G, PointRange &Base_Points, stats<indexType>
//             &QueryStats, indexType starting_point, RangeParams &QP) {
//   parlay::sequence<indexType> start_points = {starting_point};
//   return RangeSearch<Point, PointRange, indexType>(Query_Points, G,
//   Base_Points, QueryStats, start_points, QP);
// }
...
// RangeSearch(PointRange &Query_Points,
//             Graph<indexType> &G, PointRange &Base_Points, stats<indexType>
//             &QueryStats, parlay::sequence<indexType> starting_points,
//             RangeParams &RP) {

//   parlay::sequence<parlay::sequence<indexType>>
//   all_neighbors(Query_Points.size());
//   // parlay::sequence<int> second_round(Query_Points.size(), 0);
//   parlay::parallel_for(0, Query_Points.size(), [&](size_t i) {
//     parlay::sequence<indexType> neighbors;
//     QueryParams QP(RP.initial_beam, RP.initial_beam, 0.0, G.size(),
//     G.max_degree()); auto [pairElts, dist_cmps] =
//     beam_search(Query_Points[i], G, Base_Points, starting_points, QP); auto
//     [beamElts, visitedElts] = pairElts; for (indexType j = 0; j <
//     beamElts.size(); j++) {
...
//     //   all_neighbors[i] = neighbors;
//     // } else{
//     //   auto [in_range, dist_cmps] = range_search(Query_Points[i], G,
//     Base_Points, neighbors, RP);
//     //   parlay::sequence<indexType> ans;
//     //   for (auto r : in_range) ans.push_back(r.first);
//     //   if(in_range.size() > neighbors.size()) std::cout << "Range search
//     found additional candidates" << std::endl;
//     //   all_neighbors[i] = ans;
//     //   second_round[i] = 1;
...
//   // std::cout << parlay::reduce(second_round) << " elements advanced to
//   round two" << std::endl;
