...
struct Mips_JL_Point {
    ...
    float distance(const Mips_JL_Point& q) const { return pt.distance(q.pt); }

    void prefetch() const { pt.prefetch(); }

    bool same_as(const Mips_JL_Point& q) { return pt.same_as(q.pt); }
    ...
    bool operator==(const Mips_JL_Point& q) const { return pt == q.pt; }
...
struct Mips_JL_Bit_Point {
    ...
    struct parameters {
        ...
        parameters(std::vector<int8_t> const& JL_vects, int source_dims)
            ...
            std::cout << "JL dense quantization, dims = " << jl_dims
                      << std::endl;
        }
    };
    ...
    bool same_as(const Mips_JL_Bit_Point& q) { return &q == this; }
...
struct Mips_JL_Sparse_Point {
    ...
    struct parameters {
        ...
        parameters(std::vector<int8_t> const& JL_signs,
                   std::vector<int> const& JL_indices, int source_dims)
            : JL_signs(JL_signs),
              JL_indices(JL_indices),
    ...
    bool same_as(const Mips_JL_Sparse_Point& q) { return &q == this; }
...
struct Mips_JL_Sparse_Point_Normalized {
    ...
    static void translate_point(byte* values, const In_Point& p,
        ...
        const std::vector<int>& jli = params.JL_indices;
        double norm = 0.0;
        for (int j = 0; j < params.source_dims; j++) norm += p[j] * p[j];
