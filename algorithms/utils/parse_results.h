...
struct range_result {
    ...
    range_result(int nq, int nnq, double r, double r2,
                 parlay::sequence<size_t> stats, float qps, int K, int Q,
                 float c, float s)
        : num_queries(nq),
    ...
    void print() {
        std::cout << "k = " << k << ", Q = " << beamQ << ", cut = " << cut
                  << ", slack = " << slack << ", throughput = " << QPS
                  << "/second" << std::endl;
        std::cout << std::endl;
        std::cout << "Num nonzero queries: " << num_nonzero_queries
                  << std::endl;
        ...
        std::cout << "Average num visited: " << avg_visited
                  << ", 99th percentile num visited: " << tail_visited
                  << std::endl;
    }
};
...
struct lsh_result {
    ...
    lsh_result(double r, parlay::sequence<size_t> stats, float qps, int K,
               int n, long q)
