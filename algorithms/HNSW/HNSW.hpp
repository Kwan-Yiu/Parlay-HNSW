...
class HNSW {
   ...
   public:
    /*
            ...
            m_l: 				control the # of levels (larger
       m_l leads to more layer) m: 					max
       degree ef_construction:	beam size during the construction alpha:
       parameter of the heuristic (similar to the one in vamana) batch_base:
       growth rate of the batch size (discarded because of two passes)
    */
    ...
    */
    ...
    // parlay::sequence<std::tuple<uint32_t,uint32_t,float>> search_ex(const T
    // &q, uint32_t k, uint32_t ef, uint64_t verbose=0); save the current model
    // to a file
   ...
   public:
    ...
    struct dist {
        ...
        constexpr bool operator<(const dist &rhs) const { return d < rhs.d; }
    ...
    */
    ...
    static auto neighbourhood(node &u,
                              uint32_t level) -> parlay::sequence<node_id> & {
        // const constexpr auto level_none =
        // std::numeric_limits<uint32_t>::max(); return level==level_none?
        // u.final_nbh: u.neighbors[level]; return level==0? u.final_nbh:
        // u.neighbors[level];
    ...
    */
    ...
    struct graph {
        ...
        struct edgeRange {
            ...
            auto size() const { return nbh.get().size(); }
            void prefetch() const {
        ...
        decltype(auto) num_nodes() const { return hnsw.get().n; }
        ...
        uint32_t max_degree() const { return hnsw.get().get_threshold_m(l); }

        auto operator[](node_id pu) { return edgeRange(get_edges(pu)); }
        auto operator[](node_id pu) const { return edgeRange(get_edges(pu)); }
    ...
    auto select_neighbors_heuristic(
        ...
        // std::priority_queue<dist,parlay::sequence<dist>,farthest> C_cp=C,
        // W_d;
        ...
        */
        ...
        for (const auto &e : W) {
            ...
            for (const auto &r : R) {
                const auto d_r =
                    U::distance(get_node(e.u).data, get_node(r).data, dim);
    ...
    Seq prune_heuristic(Seq_ &&cand, uint32_t size, D f_dist, G g) const {
        ...
        if(ctrl.extend_nbh)
        {
                ...
                for(nid_t pc : cand_ext)
                        workset.push_back({f_dist(g.get_node(pc).get_coord()),
        pc}); cand_ext.clear();
        }
        */
        ...
        for (conn &c : workset) {
            ...
            for (const conn &r : res) {
                const auto d_cr =
                    f_dist(g.get_node(c.u).data, g.get_node(r.u).data);
                if (d_cr < d_cu) {
                    is_pruned = true;
                    break;
                }
    ...
    uint32_t get_level_random() {
        ...
        static thread_local std::mt19937 gen{
            static_cast<std::mt19937::result_type>(parlay::worker_id())};
    ...
    auto search_layer_bak(const node &u, const parlay::sequence<node_id> &eps,
                          uint32_t ef, uint32_t l_c,
                          search_control ctrl = {}) const;  // To static
    auto search_layer_new_ex(const node &u,
                             const parlay::sequence<node_id> &eps, uint32_t ef,
                             uint32_t l_c,
                             search_control ctrl = {}) const;  // To static
   ...
   public:
    ...
    uint32_t get_height() const { return get_node(entrance[0]).level; }
    ...
    /*
            void debug_output_graph(uint32_t l)
            {
                    ...
                    auto node_exist = parlay::pack(
                            node_pool,
                            parlay::delayed_seq<bool>(node_pool.size(),[&](size_t
       i){ return node_pool[i]->level>=l;
                            })
                    );
                    const auto num_vertices = node_exist.size();
                    const auto num_edges = parlay::reduce(
                            parlay::delayed_seq<uint64_t>(node_exist.size(),[&](size_t
       i){ return node_exist[i]->neighbors[l].size();
                            }),
       ...
       U::get_id(get_node(pu).data));
                            ...
                            for(node_id pv : neighbourhood(get_node(pu),l))
                                    debug_output(" %u",
       U::get_id(get_node(pv).data)); debug_output("\n");
                    }
            }
    */
    ~HNSW() {
...
HNSW<U, Allocator>::HNSW(Iter begin, Iter end, uint32_t dim_, float m_l_,
    ...
    while (batch_end < n) {
        ...
        batch_end =
            std::min({n, (uint32_t)std::ceil(batch_begin * batch_base) + 1,
        ...
        */
        ...
        insert(rand_seq.begin() + batch_begin, rand_seq.begin() + batch_end,
               true);
        // insert(rand_seq.begin()+batch_begin, rand_seq.begin()+batch_end,
        // false);
...
void HNSW<U, Allocator>::insert(Iter begin, Iter end, bool from_blank) {
    ...
    {
        parlay::sequence<parlay::sequence<std::pair<node_id, node_id>>>
            edge_add(size_batch);
        ...
        parlay::parallel_for(0, edge_add_grouped.size(), [&](size_t j) {
            ...
            // hash_table(nbh_v.begin(),nbh_v.end());
            /*
            for(auto it=nbh_v_add.begin(); it!=nbh_v_add.end();)
            {
                    bool is_extant = *it==pv||std::find_if(nbh_v.begin(),
            nbh_v.end(), [&](const node_id pu_extant){ return *it==pu_extant;
                    })!=nbh_v.end();
            ...
            */
            ...
            if (size_nbh_total > m_s) {
                ...
                for (size_t k = 0; k < nbh_v.size(); ++k)
                    candidates[k] = dist{U::distance(get_node(nbh_v[k]).data,
                                                     get_node(pv).data, dim),
                                         nbh_v[k]};
                for (size_t k = 0; k < nbh_v_add.size(); ++k)
                    candidates[k + nbh_v.size()] =
                        dist{U::distance(get_node(nbh_v_add[k]).data,
                                         get_node(pv).data, dim),
                             nbh_v_add[k]};
                ...
                /*
                auto res = select_neighbors(get_node(pv).data, candidates, m_s,
                l_c); nbh_v.clear(); for(auto *pu : res) nbh_v.push_back(pu);
                */
                // nbh_v = select_neighbors(get_node(pv).data, candidates, m_s,
                // l_c);
            } else
...
#define USE_HASHTBL
    ...
    if (ctrl.radius) {
        ...
        auto split = std::partition(
            W.begin(), W.end(), [rad](const dist_ex &e) { return e.d <= rad; });
...
auto HNSW<U, Allocator>::search_layer_new_ex(
    ...
    while (C.size() > 0) {
        ...
        */
        ...
        const auto threshold = quantile/C.size()*(dis_max-dis_min) + dis_min -
        1e-6; auto it = C.lower_bound(dist_ex{threshold,nullptr,0});
        */
                ...
                */
                ...
                if (C_acc.size() < ef || d < C_acc.rbegin()->d) {
                    ...
                    if (C_acc.size() > ef) {
                        ...
                        if (std::find_if(W_.begin(), W_.end(),
                                         [&](const dist_ex &a) {
                                             return a.u == it->u;
                                         }) != W_.end())
                            cnt_used--;
    ...
    while (not_done) {
        ...
        */
        ...
        sortedCandidates.begin(),
                        sortedCandidates.end(), new_frontier.begin(),
        dist_less);\
        */
        ...
        visited.insert(std::upper_bound(visited.begin(), visited.end(),
                                        currentPid, dist_less),
                       currentPid);
        debug_output("size of visited: %lu\n", visited.size());
        unvisited_frontier.reserve(frontier.size());
        auto uf_iter = std::set_difference(
            frontier.begin(), frontier.end(), visited.begin(), visited.end(),
...
HNSW<U, Allocator>::search_layer_to(const node &u, uint32_t ef, uint32_t l_stop,
    ...
    for (uint32_t l_c = get_node(entrance[0]).level; l_c > l_stop; --l_c) {
        ...
        // c.limit_eval = ctrl.limit_eval; // whether apply the limit to all
        // layers
...
void HNSW<U, Allocator>::save(const std::string &filename_model) const {
    ...
    const auto write = [&](const auto &data, auto... args) {
        auto write_impl = [&](auto &f, const auto &data, auto... args) {
            using T = std::remove_reference_t<decltype(data)>;
            if constexpr (std::is_pointer_v<std::decay_t<T>>) {
                auto write_array = [&](const auto &data, size_t size,
                                       auto... args) {
                    for (size_t i = 0; i < size; ++i) f(f, data[i], args...);
                };
                // use the array extent as the size
