...
auto get_reader(const char *file, const char *dir) {
    ...
    if constexpr (std::is_same_v<T, uint32_t>)
        ...
        static_assert(std::is_same_v<T, uint32_t> /*always false*/,
                      "Unsupported type");

    auto reader = [=, _ = std::move(file_h5)](T *buffer, hsize_t index,
                                              hsize_t cnt = 1) {
...
template <typename T>
std::pair<std::unique_ptr<T[]>, std::array<hsize_t, 2>> read_array_from_HDF5(
