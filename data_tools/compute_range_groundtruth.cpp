...
void write_nonzero_elts(parlay::sequence<parlay::sequence<int>> &result,
                        PointRange Query_Points, const std::string outFile) {
    ...
    parlay::sequence<int> preamble = {static_cast<int>(num_nonzero),
                                      static_cast<int>(d)};
    parlay::sequence<T> data(num_nonzero * d);
    parlay::sequence<parlay::sequence<int>> to_flatten =
    ...
    parlay::parallel_for(0, indices.size(), [&](size_t i) {
        for (int j = 0; j < d; j++)
            data[d * i + j] = Query_Points[indices[i]][j];
    });
...
void write_rangeres(parlay::sequence<parlay::sequence<int>> &result,
                    const std::string outFile) {
    std::cout << "File contains range groundtruth for " << result.size()
              << " data points" << std::endl;

    size_t n = result.size();
    parlay::sequence<int> sizes = parlay::tabulate(
        n, [&](size_t i) { return static_cast<int>(result[i].size()); });
    size_t num_matches = parlay::reduce(sizes);

    std::cout << "Number of nonzero matches: " << num_matches << std::endl;
    parlay::sequence<int> preamble = {static_cast<int>(n),
                                      static_cast<int>(num_matches)};
...
int main(int argc, char *argv[]) {
    ...
    if (tp == "float") {
        ...
        if (df == "Euclidian") {
            ...
            answers = compute_range_groundtruth<
                PointRange<float, Euclidian_Point<float>>>(B, Q, r);
        } else if (df == "mips") {
            ...
            answers =
                compute_range_groundtruth<PointRange<float, Mips_Point<float>>>(
                    B, Q, r);
        }
    } else if (tp == "uint8") {
        ...
        if (df == "Euclidian") {
            ...
            answers = compute_range_groundtruth<
                PointRange<uint8_t, Euclidian_Point<uint8_t>>>(B, Q, r);
        } else if (df == "mips") {
            ...
            answers = compute_range_groundtruth<
                PointRange<uint8_t, Mips_Point<uint8_t>>>(B, Q, r);
        }
    } else if (tp == "int8") {
        ...
        if (df == "Euclidian") {
            ...
            answers = compute_range_groundtruth<
                PointRange<int8_t, Euclidian_Point<int8_t>>>(B, Q, r);
        } else if (df == "mips") {
            ...
            answers = compute_range_groundtruth<
                PointRange<int8_t, Mips_Point<int8_t>>>(B, Q, r);
        }
    }
