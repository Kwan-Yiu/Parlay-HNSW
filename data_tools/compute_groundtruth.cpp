...
void write_ibin(parlay::sequence<parlay::sequence<pid>>& result,
    ...
    std::cout << "Writing file with dimension " << result[0].size()
              << std::endl;
...
int main(int argc, char* argv[]) {
    ...
    if (tp == "float") {
        ...
        if (df == "Euclidian") {
            ...
            answers = compute_groundtruth<PointRange<Euclidian_Point<float>>>(
                B, Q, k);
        } else if (df == "mips") {
            auto B = PointRange<Mips_Point<float>>(bFile);
            auto Q = PointRange<Mips_Point<float>>(qFile);
            answers =
                compute_groundtruth<PointRange<Mips_Point<float>>>(B, Q, k);
        }
    } else if (tp == "uint8") {
        ...
        if (df == "Euclidian") {
            ...
            answers = compute_groundtruth<PointRange<Euclidian_Point<uint8_t>>>(
                B, Q, k);
        } else if (df == "mips") {
            auto B = PointRange<Mips_Point<uint8_t>>(bFile);
            auto Q = PointRange<Mips_Point<uint8_t>>(qFile);
            answers =
                compute_groundtruth<PointRange<Mips_Point<uint8_t>>>(B, Q, k);
        }
    } else if (tp == "int8") {
        ...
        if (df == "Euclidian") {
            ...
            answers = compute_groundtruth<PointRange<Euclidian_Point<int8_t>>>(
                B, Q, k);
        } else if (df == "mips") {
            auto B = PointRange<Mips_Point<int8_t>>(bFile);
            auto Q = PointRange<Mips_Point<int8_t>>(qFile);
            answers =
                compute_groundtruth<PointRange<Mips_Point<int8_t>>>(B, Q, k);
        }
    }
