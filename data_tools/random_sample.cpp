...
void random_sample(char* iFile, int n, char* oFile) {
    ...
    std::cout << "Writing " << n << " points with dimension " << dim
              << std::endl;
...
int main(int argc, char* argv[]) {
    if (argc != 5) {
        std::cout
            << "usage: random_sample <base> <num_points_to_crop> <tp> <oF>"
            << std::endl;
        return 1;
    }
    ...
    if (tp == "float")
        random_sample<float>(argv[1], n, argv[4]);
    else if (tp == "uint8")
        random_sample<uint8_t>(argv[1], n, argv[4]);
    else if (tp == "int8")
        random_sample<int8_t>(argv[1], n, argv[4]);
    else {
